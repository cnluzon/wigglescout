...
As of version 0.20.0, `wigglescout` dropped its `future` and `furrr` dependencies
in favor of `purrr` equivalents due to some issues with mapping the partial
functions in `future` versions after 1.34.0, and the fact that in 
general this multiprocessing was not being used a lot. It might be reinstated
if I find a better solution for the mapping of the functions.
